...
//
// Кнопка стоит там же, где у посетителя на сайте (`VedalinaWidget`), —
// внизу справа, — и показывает то же лицо. Совпадение не случайно и не лень:
// по обе стороны разговора одна и та же вещь, и узнаваться она должна
// одинаково.
//
// Знак-крест с пульсирующим кольцом, стоявший здесь раньше, убран по просьбе
// владельца портала. Кольцо обещало событие, а тикало всегда — в том числе
// при пустой очереди; про «есть чему ждать ответа» говорит счётчик, и
// говорит числом.
//
// ───────────────────────────────────────────────────────────────────────────
// Что берётся откуда
  ...
  return (
    <div className="widget">
      ...
      {/* title — ради всплывающей подсказки: подпись «Разговоры» на кнопке
          ...
          На кнопке лицо Ведалины, а не знак-крест с пульсирующим кольцом.
          Кольцо владелец портала попросил убрать, и просьба верная по сути:
          пульс обещает событие, а он тикал всегда — и при пустой очереди
          тоже. Про «есть чему ждать ответа» говорит счётчик, и говорит
          числом.

          Лицо здесь по той же причине, по какой оно стоит у посетителя:
          на том конце этой кнопки — разговор, который начала Ведалина.
          Знак-крест — про VEDAL целиком и одинаков на всех кнопках портала;
          из ряда «открыть, закрыть, добавить» кнопка разговоров им
          не выделялась. */}
      <button
        type="button"
        ...
        onClick={toggle}
      >
        {/* alt пустой: подпись «Разговоры» рядом в разметке, и «портрет
            Ведалины» вторым названием той же кнопки читался бы дважды.
            eslint-disable — next/image здесь не нужен: картинка лежит
            в public/, одна на всю админку и размером в один килобайт. */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img className="widget__face" src={vedalina.avatar} alt="" width={96} height={96} />
