...
export function Palette({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    // Клиенты — контур продаж. Соседние двери выше по файлу спрашиваются
    // по роли, а эта спрашивалась всегда: у редактора сайта портал отвечал
    // отказом на КАЖДОЕ нажатие клавиши. Отказ проглатывался catch'ем,
    // поэтому на экране ничего не менялось, — но в журнале портала оседала
    // очередь отказов, а человек искал в двери, которая ему не открыта.
    if (!продажи) return;
