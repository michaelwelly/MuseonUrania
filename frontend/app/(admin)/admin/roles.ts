...
/**
 * Все портальные роли — в том порядке, в каком их показывают.
 *
 * Порядок от общего к частному: администратор, потом два контура.
 * Тот же порядок держит бэкенд (StaffDirectory.PORTAL_ROLES), и списки
 * обязаны совпадать: роль, которой нет здесь, не покажется в редакторе,
 * а роль, которой нет там, портал отвергнет при сохранении.
 */
export const PORTAL_ROLES = [ADMIN, SALES, PRODUCTION] as const;
