...
describe("разделы по ролям", () => {
  ...
  it("администратор видит всё", async () => {
    ...
    for (const раздел of ["Сводка", "Содержимое сайта", "Работа с клиентами", "Сотрудники", "Журнал"]) {
      expect(разделы()).toContain(раздел);
    }
  });

  // Раньше здесь была «Команда» с двумя вкладками, и у продавца раздел
  // состоял из одного пункта — его собственного профиля. Профиль ушёл
  // в кружок с именем, а разделом осталось то, что действительно раздел:
  // состав компании. Он административный.
  it("состав компании — раздел administратора, а не всех подряд", async () => {
    await оболочка(["portal-sales"]);

    expect(разделы()).not.toContain("Сотрудники");
  });

  // Профиль остаётся достижимым для КАЖДОЙ роли — просто не из навигации.
  // Без этой проверки уборка вкладки могла бы запереть человека без
  // единственного пути к собственной странице, и заметили бы это не сразу:
  // адрес-то работает, если его знать.
  it("в свой профиль ведёт кружок с именем, у любой роли", async () => {
    for (const роль of ["portal-admin", "portal-sales", "portal-production"]) {
      cleanup();
      await оболочка([роль]);

      // Слеш на конце в разметке не гарантирован: trailingSlash работает
      // на маршрутах, а не на том, что Link кладёт в href.
      const ссылки = [...document.querySelectorAll('a[href^="/admin/profile"]')];
      expect(ссылки.length).toBeGreaterThan(0);

      // И это НЕ пункт навигации: иначе проверка зеленела бы на вернувшейся
      // вкладке «Мой профиль».
      expect(разделы()).not.toContain("Мой профиль");
    }
  });
});
