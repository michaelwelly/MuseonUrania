  ...
  return (
    <>
      ...
      <p className="admin-hint">
        Здесь хранится открытый пакет документов VEDAL. На сайте показываются только
        строки с отметкой «в перечне», а кнопка скачивания появляется после загрузки
        файла и публикации. Ведалина берёт в поиск опубликованные PDF; после замены
        файла индекс обновляется автоматически, а ручная кнопка нужна для контрольной
        пересборки. Предел файла — {MAX_MB} МБ.
      </p>
...
function KnowledgeIndex({
  ...
  return (
    <div className="admin-card">
      ...
      {state && !state.enabled && (
        <p className="muted">
          Индекс по PDF пока выключен. Ведалина отвечает по опубликованным материалам
          сайта обычным поиском; после включения RAG здесь появится список документов,
          которые попали в поиск по смыслу.
        </p>
      )}

      {state?.enabled && (
        <p className="muted">
          Сейчас в индексе <span className="mono">{state.sources}</span>{" "}
          {склонение(state.sources, "материал", "материала", "материалов")} и{" "}
          <span className="mono">{state.chunks}</span>{" "}
          {склонение(state.chunks, "фрагмент", "фрагмента", "фрагментов")}. Если
          документ заменили, а Ведалина отвечает по старой версии, нажмите
          «Переиндексировать».
        </p>
      )}
    </div>
