...
//
// ───────────────────────────────────────────────────────────────────────────
// Про роли сказана правда, а не иерархия
//
// В макете «что можно с этими ролями» — четыре выполненных пункта и один
// с прочерком, как будто у администратора и редактора права разные. У портала
// они одинаковые: `SecurityConfig` пускает обе роли ко всему админскому API
// одним правилом, и это записано там осознанно — «разделение на „кто читает“
// и „кто правит“ появится вместе с CRM, заводить его сейчас значит выдумать
// иерархию, которой никто не пользуется».
//
// Показать здесь разные права двух ролей значило бы дать сотруднику ложное
// чувство границы: он решит, что редактор чего-то не может, и оставит ему
// доступ, которого не собирался давать.
//
// ───────────────────────────────────────────────────────────────────────────
    ...
    <>
      ...
      <div className="board2">
        <section>
          ...
          <div className="admin-card">
            <ul className="check">
              <Can yes>Править каталог, новости и документы, публиковать и снимать</Can>
              <Can yes>Вести заявки, клиентов, сделки и КП, отвечать в разговорах</Can>
              <Can yes>Уничтожать персональные данные по обращению субъекта</Can>
              <Can yes>Читать журнал целиком</Can>
              <Can>
            ...
            <p className="admin-hint">
              Роли <code>portal-admin</code> и <code>portal-editor</code> сегодня дают
              одно и то же: портал пускает обе ко всему админскому API одним правилом.
              Разделение на «кто читает» и «кто правит» ещё не заведено, и показывать
              его здесь значило бы дать ложное чувство границы.
            </p>
          </div>
        </section>
