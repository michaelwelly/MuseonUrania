...
const NAV: readonly Section[] = [
  ...
  },
  {
    ...
    items: [
      ...
      // Дежурство сразу за разговорами: это ответ на вопрос «кто сегодня
      // отвечает», и задают его, стоя в разговорах. Счётчика нет —
      // число «сколько там дежурства» ничего не значит.
      { href: "/admin/duty/", label: "Дежурство" },
    ...
    <div className="admin-shell">
      ...
      <div className="admin-nav" ref={nav}>
        <div className="admin-bar">
          {/* Знак ведёт на публичный сайт, а не на сводку админки: так
              просил заказчик — логотип для него это «домой», а дом здесь
              сайт, а не рабочее место.

              В новой вкладке, и это не украшение. Редактор правит карточку
              изделия и машинально жмёт логотип; уведи его из админки в том
              же окне — и незаполненная форма пропадёт вместе с ней. Правая
              кнопка «Открыть сайт» открывается так же, поэтому поведение
              у обеих дорог на сайт одинаковое.

              Сводка от этого не потерялась: она первым пунктом навигации
              рядом. */}
          <Link
            ...
            target="_blank"
            rel="noopener"
            aria-label="Открыть публичный сайт VEDAL в новой вкладке"
          >
      ...
      {/* Компактная рабочая подсказка и два быстрых действия. */}
      <footer className="admin-foot">
        <span className="admin-foot__circuit">подсказка</span>
        <span className="admin-foot__say">
          Используйте поиск в шапке для быстрого перехода. Заявки, контакты и история
          работы хранятся в портале.
        </span>
