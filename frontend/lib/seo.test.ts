...
describe("метаданные страницы", () => {
  ...
  it("без своей картинки берёт карточку сайта и широкий формат", async () => {
    ...
    //
    // Метка версии в адресе обязательна: без неё мессенджер отдаёт из кэша
    // ту карточку, которую утащил при первой встрече со ссылкой, и новую
    // не запрашивает. Проверяется вместе с путём — метка, потерянная при
    // правке, иначе всплывёт только в чужом кэше, где её уже не достать.
    expect((meta.openGraph as { images: { url: string }[] }).images[0].url)
      .toBe("/brand/og-cover.png?v=2");
