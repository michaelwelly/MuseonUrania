...
// Шапка окна. Issue #106.
//
// Рядом стояли «−» («Свернуть чат») и «×» («Закрыть чат»), и обе вызывали
// один и тот же обработчик. Проверка живёт здесь, потому что сломать это
// ничего не стоит: вторая кнопка возвращается одной строкой разметки,
// а видно её только тому, кто сравнит подписи с поведением.

describe("кнопка закрытия", () => {
  it("одна, а не две одинаковые с разными подписями", async () => {
    const закрыть = vi.fn();
    await act(async () => {
      render(<VedalinaChat onClose={закрыть} />);
    });

    // Две кнопки, обещающие разное и делающие одно, — это выбор,
    // которого нет.
    expect(screen.queryByRole("button", { name: "Свернуть чат" })).toBeNull();
    const кнопки = screen.getAllByRole("button", { name: "Закрыть чат" });
