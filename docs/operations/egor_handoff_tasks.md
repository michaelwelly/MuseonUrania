# Задачи Егора и разделение работ

**Русский** · [English](egor_handoff_tasks.en.md)

## Назначение

Документ фиксирует текущую передачу работ Егору и отделяет поставку фронтенда от
работ по бэкенду, CMS и закрытому AI-контуру.

Егор отвечает за UX/UI и реализацию фронтенда. Михаил отвечает за бэкенд и CMS,
инфраструктуру, архитектуру ИИ, архитектурные ограничения фронтенда и итоговую
приёмку интеграции.

## Договорённость о работе

- Егор пишет фронтенд по утверждённому визуальному направлению, опираясь на этот
  репозиторий как на исходный пакет.
- Михаил даёт архитектуру, контракты API, структуру контента, объём работ по
  бэкенду и CMS и ревью интеграции.
- НН утверждает структуру и деловые формулировки со стороны заказчика.
- Публичные заявления о медицинских изделиях, цены, сертификаты и статус
  регистрации выдумывать нельзя.
- Недостающий контент помечается как `ожидает уточнения`.

## Стартовый пакет для Егора

- `HANDOFF.md`
- `docs/frontend/README.md`
- `docs/frontend/claude_context_bundle.md`
- `docs/frontend/sitemap.md`
- `docs/frontend/content_model.md`
- `docs/frontend/page_briefs.md`
- `docs/frontend/implementation_checklist.md`
- `docs/strategy/vedalina_assistant_spec.md`
- `docs/strategy/vedalina_visual_assets.md`
- `assets/vedalina/vedalina-avatar-middle-v1.png`
- `assets/vedalina/vedalina-web-integration-mockup-v1.png`
- `assets/brand/`
- Клиентские презентации, VM-карта и доступы хранятся вне публичного git в
  официальном пакете проекта.

## Текущий стенд

- Стенд-продакшн: `http://51.250.31.97:18080`
- ВМ: `smart_soultion_mvp`
- Команда деплоя: `ssh -p 2222 ubuntu@51.250.31.97 '/opt/vedal-portal/scripts/deploy-stand-prod.sh main'`
- C3AG живёт на той же ВМ и не должен затрагиваться фронтовыми правками.
- VEDAL использует Yandex Object Storage:
  - публичные медиа-объекты сайта: `vedal-media`;
  - приватные документы: `vedal-documents`.

## SSH-доступ Егора

Если у Егора `ssh -p 2222 ubuntu@51.250.31.97` завершается `connection timeout`,
это не проблема ключа. На ВМ проверено:

- `sshd` слушает `0.0.0.0:2222`;
- ключ `asura@DESKTOP-OTGUHVS` добавлен в `/home/ubuntu/.ssh/authorized_keys`;
- `ufw` выключен;
- локальный `iptables` не блокирует вход.

Значит доступ режется снаружи, на уровне Yandex Cloud Security Group или сети
Егора. Нужно добавить inbound rule:

```text
Protocol: TCP
Port: 2222
Source: <публичный IP Егора>/32
Description: Egor SSH deploy access
```

На 2026-08-18 добавлено правило для VPN/public egress IP Егора:

```text
Protocol: TCP
Port: 2222
Source: 38.68.134.13/32
Description: egor-ssh-deploy-2222-20260818
```

Адрес `192.168.100.134` не подходит для Security Group: это приватный локальный
адрес внутри сети Егора, не публичный адрес, с которого подключение приходит в
Yandex Cloud.

Порт `22` не используем. Открывать `0.0.0.0/0` для SSH можно только временно и
после согласования. Порты `80` и `443` заняты C3AG, порт `18080` остается
публичным стендом VEDAL.

## Задачи фронтенда для Егора

### P0. Первый экран и навигация

- Пересобрать первый экран по утверждённому визуальному референсу PNG.
- Сохранить VEDAL основным брендом, Smart Solution — вторичным интеграционным слоем.
- Добавить кнопку `Войти` слева от знака VEDAL.
- Добавить `Новости` в шапку между `Документы` и `Контакты`.
- Добавить `Сервис` рядом с `Продукция`.
- Сохранить телефон и основной призыв к действию видимыми в десктопной шапке.
- Подготовить адаптивную мобильную шапку.

### P0. Интерфейс публичного ассистента Ведалина

- Переименовать Ведалину в Ведалину во всём публичном UI.
- Использовать утверждённый второй, средний вариант аватара.
- Добавить карточку Ведалины на первый экран.
- Добавить плавающую кнопку Ведалины в правом нижнем углу.
- Подпись публичного режима: `AI-поиск по открытым документам`.
- Быстрые действия:
  - `Подобрать оборудование`
  - `Найти документ`
  - `Запросить КП`
  - `Сервис и поддержка`
  - `Получить каталог`
- Интерфейс публичного ассистента не должен подразумевать диагностику,
  рекомендации по лечению или поддержку клинических решений.

### P0. Основные страницы

- Главная.
- Страница каталога продукции.
- Шаблон карточки изделия для 4 публичных продуктов:
  - `VEDAL A-2000`;
  - `VEDAL R1`;
  - `VEDAL R2`;
  - `VEDAL Т-100`.
- Страница сервиса и гарантии.
- Страница документов.
- Страница новостей с заглушкой под релиз Иннопрома.
- Страница контактов.

### P0. Правки по итогам встречи с заказчиком

- О компании:
  - убрать блок цифр `12 / 5 / 1 / 3`;
  - оставить фото и рассказ о компании;
  - добавить фокус на собственный отдел разработки;
  - убрать блок `Направления`;
  - убрать блок `Партнёры`;
  - переименовать `Похожие модели` в `Другие продукты`;
  - вместо партнёров добавить блок членства в Уральской ТПП.
- Продукция:
  - оставить только `VEDAL A-2000`, `VEDAL R1`, `VEDAL R2`, `VEDAL Т-100`;
  - разделить R1 и R2 на отдельные страницы;
  - теги категорий пока оставить.
- Карточки изделий:
  - убрать кнопку `Описание изделия`;
  - текущие описания и характеристики считать временными до текстов от НН.
- Производство:
  - убрать кнопку `Записаться на визит`;
  - заменить `Участок` на `производственная площадка`;
  - убрать блок про сборочные места, отладку, хранение и приём посетителей;
  - убрать блок качества.
- Контакты:
  - оставить только `8 800 600 3449`, пока заказчик не подтвердит остальные телефоны;
  - сотрудников и прямые контакты оставить как ожидающие подтверждения.
- Общие визуальные правки:
  - добавить очень прозрачный фоновой паттерн с крестиками из буклета;
  - определить место для знака дерева, который наносится на изделия;
  - задачу запрета копирования вынести в отдельное техническое решение.

### P1. Формы

- Форма запроса КП.
- Форма запроса каталога.
- Форма заявки на сервис.
- Форма консультации и обратной связи.
- Форма партнёрского обращения, если будет согласована.
- Формы должны быть устроены под передачу в бэкенд и CRM.
- В каждой форме должен быть текст согласия и приватности до отправки.

### P1. Структура контента

- Карточки поддерживают 4 продукта MVP и не показывают несогласованные изделия.
- Поля данных изделия используют заглушки, пока НН не утвердит каталог.
- В документах показываются только публичные материалы.
- Новости поддерживают как минимум один пресс-релиз Иннопрома.
- Условия гарантии и сервиса появляются на странице сервиса только после
  утверждения заказчиком.

### P1. Готовность к SEO и аналитике

- Заголовок страницы, описание, h1, canonical, поля OG.
- Структурированные адреса для продукции, документов, новостей и сервиса.
- Имена событий фронтенда согласованы с
  [чек-листом реализации](../frontend/implementation_checklist.md).
- Подготовить точки подключения событий Яндекс Метрики.

### Критерии приёмки для Егора

- Десктопная и мобильная раскладки стабильны.
- Пункты шапки не переносятся некрасиво.
- Ведалина заметна, но не перебивает сообщение о продукции и производстве.
- Карточки изделий и категорий не съезжают, когда часть контента отсутствует.
- У всех кнопок есть внятные состояния.
- У форм есть состояния валидации, успеха и ошибки.
- Нет фальшивых цен, сертификатов, клинических заявлений и закрытых документов.

## Объём работ по бэкенду и CMS для Михаила

### CMS и админка

- Вход администратора и доступ в закрытый контур.
- Управление страницами.
- Управление продукцией.
- Управление документами.
- Управление новостями.
- Управление SEO-метаданными.
- Управление флагами публичной и закрытой видимости.
- Подготовить роли редакторов и журнал аудита.

### API

- Контракт Swagger/OpenAPI.
- Публичное API для опубликованного контента сайта.
- Forms API для заявок на КП, каталог, сервис и обратную связь.
- Admin API для операций CMS.
- Assistant API для публичного и авторизованного режимов Ведалины.
- Проверки живости и базовые endpoint'ы мониторинга.

### Хранилище

- S3-совместимое хранилище.
- Публичный бакет или пространство имён для изображений, публичных документов,
  каталогов и пресс-материалов.
- Приватный бакет или пространство имён для внутренней документации.
- Метаданные в базе: тип документа, область доступа, язык, связь с изделием,
  статус публикации.

### Аутентификация и закрытый контур

- Аутентификация для админов и сотрудников.
- Ролевой доступ к внутренним страницам и документам.
- Публичный сайт отделён от админки, внутренних документов и AI-поиска.
- Персональные и деловые данные хранятся в инфраструктуре Российской Федерации.

### Передача в CRM

- Модель лида для запросов КП, каталога, консультации, сервиса и партнёрства.
- Резервное уведомление по почте.
- Готовый к выгрузке и интеграции формат данных под будущую CRM.
- Поля UTM и источника для рекламы и аналитики.

## Объём работ по ИИ и закрытому контуру для Михаила

### Публичная Ведалина

- Публичная Ведалина отвечает только по согласованным публичным страницам и
  документам.
- Она умеет вести по продукции, документам, сервису и коммерческим шагам.
- При низкой уверенности направляет обращение в форму или к живому специалисту.
- Даёт ссылки на источники там, где это возможно.

### Авторизованная Ведалина

- После входа Ведалина может искать по внутренним документам и базам.
- Ответы фильтруются по роли пользователя и правам на документы.
- Внутренние ответы не должны просачиваться в публичный режим.
- Все внутренние обращения к ИИ должны быть пригодны для аудита.

### Обработка документов

- Классифицировать документы как `public`, `internal`, `confidential`, `service`,
  `production` или `R&D`.
- Извлекать текст из документов.
- Нарезать на чанки с метаданными.
- Строить эмбеддинги.
- Хранить и искать через PostgreSQL + pgvector или другой согласованный стек.
- Сохранять ссылки на исходные документы и версии.

### Жёсткие правила безопасности

- Никаких диагнозов.
- Никаких рекомендаций по лечению.
- Никаких выдуманных цен.
- Никаких выдуманных сертификатов и статусов регистрации.
- Никаких выдуманных сроков поставки и наличия.
- Никакого публичного использования закрытых и внутренних документов.
- Никаких ответов из несогласованных R&D-материалов и конфиденциальных
  производственных материалов.

## Разделение бэклога

### MVP

- Редизайн публичного сайта.
- Обновление шапки и навигации.
- Структура каталога продукции.
- Страницы публичных документов, новостей и сервиса.
- Оболочка публичного интерфейса Ведалины.
- Формы и структура передачи в CRM.
- Минимально достаточный объём CMS и админки.
- Яндекс Метрика и баннер согласия.

### Фаза 2

- Полная интеграция с CRM.
- Полный жизненный цикл документов в S3.
- Авторизованный поиск по внутренним документам.
- Оценка качества RAG.
- Матрица доступа сотрудников.
- Мультиязычный контент на русском, английском и китайском.

### Фаза 3

- Локальное или приватное развёртывание LLM.
- Пилот VLM для сервиса и производства.
- Расширенная внутренняя аналитика.
- Полный корпоративный ИТ-контур на 60 сотрудников.

## Ближайшие шаги

1. Егор начинает фронтенд с утверждённого первого экрана и чек-листа.
2. Михаил готовит контракты API и модель данных CMS.
3. НН подтверждает список продукции, документы, условия сервиса и публичные
   формулировки.
4. Команда смотрит первый развёрнутый стенд до того, как уходить глубже в
   интеграцию с бэкендом.

---

## Состояние на 7 сентября 2026

Сверено с кодом и с живым стендом, а не по памяти. Каждая строка ниже —
результат проверки, а не намерение.

### P0 — закрыто

| Пункт | Чем подтверждается |
| --- | --- |
| Первый экран и навигация | Шапка с «Продукция», «Сервис», «Документы», «Новости», «Контакты», «О компании», телефон и призыв «Запросить КП» — все на месте на живой главной |
| Мобильная шапка | На 375 px ни одна из десяти страниц не даёт горизонтальной прокрутки (замер `scrollWidth − clientWidth = 0`) |
| Интерфейс Ведалины | Карточка на первом экране, плавающая кнопка, быстрые действия, подпись «Задать вопрос Ведалине» |
| Основные страницы | Десять маршрутов плюс политика приватности; каталог, новости и документы приходят из портала, а не лежат в коде |
| Правки по итогам встречи | Блок цифр, «Направления» и «Партнёры» убраны, вместо партнёров — членство в Уральской ТПП; фоновый паттерн добавлен; знак дерева на месте |

### P1 — закрыто, кроме зависимостей

| Пункт | Состояние |
| --- | --- |
| Формы | Четыре из пяти: КП, каталог, консультация, сервис. Партнёрская ждёт согласования — её не заказывали |
| Структура контента | Четыре изделия, документы только публичные, новости поддерживаются — но ни одной не опубликовано, материалов нет |
| SEO | Карта сайта и `robots.txt` собираются из портала, canonical и Open Graph на каждой странице |
| События аналитики | Все девять имён из чек-листа размечены в коде |
| Яндекс Метрика | Не подключена: заказчик не подтвердил, можно ли ставить счётчик |

### Критерии приёмки — проверены

Десктоп и мобильный устойчивы, пункты шапки не переносятся некрасиво,
карточки не съезжают при отсутствии части данных, у форм есть состояния
проверки, успеха и ошибки. Выдуманных цен, сертификатов и клинических
заявлений нет — это держится не обещанием, а правилами до движка
(`Guardrails`) и тестами.

### Что не сделано и почему

Всё оставшееся упирается в материалы, доступы и решения заказчика: фото и
видео, финальные тексты и разрешения на публикацию, домен и HTTPS, почтовый
ящик, реквизиты и контакты, срок хранения заявок, Метрика, языки, 1С.
Сведено одним списком в «customer_checklist.md» (`docs/requests/customer_checklist.md` — вынесен из публичного репозитория, см. [чистку 8 сентября](../security/public_repo_cleanup_2026-09-08.md)).

## Состояние на 8 сентября 2026

Днём позже, перед показом заказчику. Что изменилось за сутки.

### Правки по итогам просмотра стенда

Заказчик прошёл по стенду 8 сентября и назвал девять правок. Восемь сделаны:

| Правка | Как сделано |
| --- | --- |
| КПП 668601001 везде, где есть реквизиты | одно место в коде, на страницы приходит оттуда |
| УТПП не должна быть ссылкой | членство осталось текстом, в том числе в подвале |
| Кнопки соцсетей убрать до появления ссылок | скрыты; вернутся, когда каналы подтвердят (§13.5) |
| Часы работы 9:00–17:30 | и на сайте, и в ответах портала о присутствии на линии |
| Убрать вкладку «Комплектация» на четырёх карточках | вкладки рисует один общий компонент, правка одна на все |
| Карта на контактах | встроенной карты нет — iframe ставит сторонние cookie; работает ссылка «Построить маршрут» на Яндекс.Карты по адресу из карточки компании |
| Портрет Ведалины в блоке «Быстрее — через Ведалину» | тот же портрет, что в чате, берётся из карточки ассистента |
| Иконка вкладки браузера | favicon, иконка приложения и иконка для iOS |

Девятая — ссылки «Описание» на реальные файлы каталога — ждёт самих файлов
и разрешения на публикацию.

### Почта

Ящик получен, транспорт проверен снаружи: `mail.vedal-med.ru`, порт 465
с неявным TLS, вход принят. Кода правки не потребовали — этот транспорт уже
был зашит. Осталось вписать три переменные на машине,
см. [mail_setup.md](mail_setup.md).

### Домен

`vedal-med.ru` существует, зона у `nic.ru`, и A-запись **уже указывает
на нашу машину**. Почта домена при этом на отдельном хосте и правкой
A-записи не затрагивается. Что отвечает на 80/443 по этому имени, предстоит
выяснить на самой машине, см.
[domain_cutover_vedal_med_ru.md](domain_cutover_vedal_med_ru.md).

### Метрика

Счётчик подключается двумя условиями сразу: задан номер и посетитель принял
плашку согласия. Плашка сделана, номер счётчика — за заказчиком.

### Что мешает прямо сейчас

Доступ к машине по SSH закрыт: внешний адрес разработчика сменился,
а в группе безопасности стоит прежний. Пока правило не обновят, на стенде
нельзя ни настроить почту, ни проверить домен, ни убедиться, что автодеплой
довёз последний `main`.
