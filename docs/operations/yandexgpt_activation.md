...
Конвейер собран целиком: текст опубликованных PDF и DOCX извлекается,
событие о правке документа поднимает переиндексацию, в админке есть кнопка
и состояние индекса. Не хватает единственного — корпуса: пока в индекс
попадает то, что портал и так показывает, — изделия, новости и карточки
документов. Модель не читает закрытые файлы: материал уровня `confidential`
в индекс не попадает вовсе, его нет даже значением в схеме.

Что осталось: калибровка порога близости. Умолчание 0.45 заведомо неточно,
и мерить его можно только на настоящих документах и настоящих вопросах —
порядок замера описан в
[конвейере Ведалины](vedalina_rag_pipeline.md#как-калибровать-порог-когда-появится-корпус).
