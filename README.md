...
Нужен только Docker — ни Java, ни Node, ни Maven ставить не надо. Всё
остальное собирается в контейнерах.
...
Скрипт проверит Docker, сгенерирует `backend/.env` со случайными паролями,
соберёт образы, дождётся готовности и напечатает адреса. **Первый запуск —
около десяти минут:** Maven тянет зависимости, Next собирает страницы. Дальше
секунды, слои кэшируются.

| Адрес | Что |
| --- | --- |
| `http://localhost:8080` | сайт |
| `http://localhost:8080/admin/` | админка |
| `http://localhost:8080/swagger-ui.html` | спецификация API |
| `http://localhost:8180` | Keycloak |

Вход в админку — `editor` / `editor-local`. Это учётная запись локального
стека, она заведена импортом realm'а и на сервер не уезжает; почему пароль
лежит в репозитории, разобрано в [backend/keycloak/README.md](backend/keycloak/README.md).

Остановить, не теряя данные:

```bash
docker compose -f backend/compose.yaml --profile app down
```

Что делать, если что-то не поднялось, как запускать приложения с машины
вместо контейнеров и чем отличается режим разработки — в
[docs/PROJECT.md, раздел 5.8](docs/PROJECT.md#58-окружения-и-запуск).
...
- `db` — миграции и seed-данные, вливается в `dev`;
- `infra` — сборка, CI/CD, Docker, деплой, вливается в `dev`;
- `docs` — документация, вливается в `dev`.

Правила ведения документации — [docs/documentation_rules.md](docs/documentation_rules.md),
правила работы с ветками — [docs/PROJECT.md, раздел 4](docs/PROJECT.md#4-ветки).
