...
public record AskReply(
    ...
    public record Handoff(
            ...
            @Schema(description = """
                    Ставить ли разговор в очередь к живому специалисту.

                    `false` означает «контакты показать, человека не звать»: вопрос,
                    по которому не нашлось материалов, специалисту нечего продолжать,
                    а очередь, куда падает каждый такой разговор, дежурный перестаёт
                    читать. `true` — цена, сроки, наличие, статус регистрации,
                    клинический вопрос, жалоба и прямая просьба позвать живого.
                    """, example = "true")
            boolean queue) {}
}
