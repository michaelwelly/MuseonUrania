...
public class AssistantService {
    ...
    /**
     * Ответа нет — но разговор не заканчивается.
     *
     * <p>Раньше здесь стояло «отвечать не буду, передам специалисту», и это
     * был тупик: посетитель, спросивший не теми словами — «а грелка для
     * малыша есть?» вместо «система терморегуляции», — получал отказ и уходил.
     * Передача человеку остаётся (она в {@code handoff} рядом), но перед ней
     * стоит вопрос: может быть, речь про наше оборудование, только названное
     * иначе.
     *
     * <p>Это именно вопрос, а не подмена запроса. «Вы имели в виду инкубатор»
     * с готовым ответом по инкубатору — ответ на незаданный вопрос, и
     * с медицинским оборудованием так нельзя. Здесь выбор делает человек.
     */
