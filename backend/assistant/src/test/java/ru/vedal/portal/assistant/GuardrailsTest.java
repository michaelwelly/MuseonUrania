...
class GuardrailsTest {
    ...
    //
    // 16 сентября выяснилось худшее следствие того же: отказ по цене САМ
    // ставил разговор в очередь, после чего ассистент для этого посетителя
    // замолкал навсегда. Отказ обязан сказать, чего Ведалина не говорит,
    // и остаться в разговоре — человека зовёт тот, кто его просит.
    @Test
    void aRefusalDoesNotCallASpecialistByItself() {
        assertThat(guardrails.refuse("сколько стоит инкубатор").orElseThrow().human())
                .as("цену не назвали — но разговор остаётся у Ведалины")
                .isFalse();
        assertThat(guardrails.refuse("когда поставите R2").orElseThrow().human())
                .as("сроки подтверждает специалист, и просят его отдельно")
                .isFalse();
        assertThat(guardrails.refuse("есть ли регистрационное удостоверение")
                .orElseThrow().human())
                .as("статус регистрации ассистент не подтверждает и очередь не заводит")
                .isFalse();
    ...
    // А вот это — те два случая, где передача и есть ответ.
    @Test
    void aPersonIsCalledWhereTheyAreAskedForOrNeeded() {
        assertThat(guardrails.refuse("позовите специалиста").orElseThrow().human())
                .as("попросили человека — человека и зовём")
                .isTrue();
    ...
    // Обещание в тексте обязано совпадать с поведением. Пока отказ сам ставил
    // разговор в очередь, «я передам специалисту» было правдой; теперь нет —
    // и в тексте остались только те два пути, которые работают на самом деле:
    // форма на сайте и просьба позвать живого словами.
    @Test
    void aRefusalPromisesOnlyWhatActuallyHappens() {
        var price = refusalText("сколько стоит инкубатор").orElseThrow();
        assertThat(price)
                .as("обещания передачи, которой не будет, в тексте нет")
                .doesNotContain("я передам специалисту")
                .as("зато сказано, как позвать живого и как оставить обращение")
                .contains("напишите об этом, и я передам разговор")
                .contains("формой на сайте")
                .as("и что разговор продолжается")
                .contains("про сами изделия");

        assertThat(refusalText("когда поставите R2").orElseThrow())
                .doesNotContain("я передам специалисту")
                .contains("напишите об этом, и я передам разговор");

        assertThat(refusalText("есть ли регистрационное удостоверение").orElseThrow())
                .as("недостающее — «ожидает уточнения», а не правдоподобная выдумка")
                .contains("ожидает уточнения")
                .contains("и я передам разговор");
    }
}
