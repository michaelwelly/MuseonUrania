    ...
    static HttpSecurity adminApiBaseline(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // Выдача ролей — тоже раньше правила контура,
                        // и по той же причине, что уничтожение данных:
                        // /staff/{логин}/roles лежит под /staff/**, а тот
                        // открыт любой портальной роли — справочник нужен
                        // всем для выбора ответственного. Без этой строки
                        // продавец выдавал бы роли сам себе и кому угодно.
                        //
                        // Порядок здесь — не стиль, а само правило: Spring
                        // берёт ПЕРВОЕ совпавшее, и строка, переставленная
                        // ниже справочника, не запретит ничего и не скажет
                        // об этом. Обе стороны закреплены в RoleMatrixTest.
                        .requestMatchers("/api/admin/v1/staff/*/roles")
                        .hasRole(ROLE_ADMIN)
                        ...
                        // Свой портрет. Роль здесь любая портальная по той же
                        // причине, что у /session: это «я», а не раздел, и
                        // закрыв дверь одной ролью, мы отобрали бы у половины
                        // сотрудников их собственную картинку.
                        //
                        // Ветка адресов отдельная и без логина в пути — не для
                        // порядка: параметра, в котором можно назвать чужой
                        // логин, здесь нет вовсе, и правило «меняешь только
                        // своё» держится формой адреса, а не проверкой,
                        // которую можно забыть. Чтение чужого портрета живёт
                        // под /staff/** — это чтение справочника.
                        .requestMatchers("/api/admin/v1/profile/**")
                        .hasAnyRole(ROLE_ADMIN, ROLE_SALES, ROLE_PRODUCTION)
                        ...
                        //
                        // Сначала он был закрыт администратором, и это была
                        // ошибка: без него продажи не выберут ответственного
                        // ни у заявки, ни у сделки — поле «ответственный»
                        // берёт список отсюда. Он же превращает логин в имя
                        // в приветствии и в журнале.
                        //
                        // Отдавать его безопасно: логин, имя и признак
                        // «учётная запись включена». Ни клиентов, ни сумм,
                        // ни персональных данных посетителей здесь нет,
                        // а состав компании сотрудник и так видит в почте.
                        //
                        // Экран «Сотрудники» с нагрузкой — другое дело:
                        // он показывает, сколько на ком заявок и сделок,
                        // и остаётся административным. Закрывает его
                        // интерфейс, а числа для него всё равно берутся
                        // из дверей контура продаж.
                        .requestMatchers("/api/admin/v1/staff/**", "/api/admin/v1/staff")
                        .hasAnyRole(ROLE_ADMIN, ROLE_SALES, ROLE_PRODUCTION)

                        // ————— только администратор —————
                        //
                        // Журнал показывает, кто что делал, — включая тех,
                        // кто смотрит. Для работы контуров он не нужен.
