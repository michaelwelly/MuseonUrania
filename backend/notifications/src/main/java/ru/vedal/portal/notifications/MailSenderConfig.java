...
public class MailSenderConfig {
    ...
    MailSender portalMailSender(ObjectProvider<JavaMailSender> transport,
        ...
        if (javaMail == null || host.isBlank()) {
            ...
            // разработчика и стенда.
            //
            // Текст этой строки однажды уже разошёлся с делом. Он обещал, что
            // письма «помечаются отправленными», — так было до issue #46,
            // и после починки предупреждение осталось прежним: портал держал
            // письма в очереди, а в журнал писал, что потерял их. Ложь в свою
            // пользу заметить труднее всего, поэтому теперь строка говорит
            // ровно то, что делает configured() = false ниже.
            log.warn("SMTP не настроен (нет spring.mail.host). Письма остаются"
                    + " в очереди со статусом queued и уйдут, как только почта"
                    + " появится. Для отправки задайте SPRING_MAIL_HOST,"
                    + " SPRING_MAIL_USERNAME и SPRING_MAIL_PASSWORD.");
