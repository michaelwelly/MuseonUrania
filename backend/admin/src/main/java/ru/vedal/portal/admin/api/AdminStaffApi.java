...
public class AdminStaffApi {
    ...
    public List<StaffDirectory.Person> assign(@PathVariable String login,
        ...
        //
        // Запись СВОЕЙ транзакцией, а не общей, и это здесь не выбор
        // из двух одинаковых. `record` объявлен MANDATORY — он пишет
        // в транзакции вызывающего, чтобы запись журнала и действие
        // коммитились вместе. У выдачи ролей коммитить не с чем:
        // изменение живёт в Keycloak, а не в базе портала, и транзакции
        // вокруг обработчика нет вовсе. MANDATORY в такой обстановке
        // не «пишет как обычно», а бросает IllegalTransactionStateException
        // ПОСЛЕ того, как роль уже выдана: сисадмин видит 500, роль при
        // этом изменена, а в журнале её изменения нет — то есть ровно
        // тот случай, ради которого запись здесь и стоит (issue #95).
        audit.recordIndependently(actor, "staff.roles", "staff", login,
