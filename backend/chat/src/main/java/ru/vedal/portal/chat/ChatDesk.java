...
// ————— главное правило —————
//
// Как только человек в разговоре, Ведалина замолкает. Не «отвечает реже»
...
// ————— и его граница —————
//
// «Человек в разговоре» — это взятый разговор или открытое рабочее место,
// а не сам факт очереди. Разговор, который никто не взял и брать сейчас
// некому, человеку не передан — он в неё положен. Перебивать там некого,
// а посетитель при этом упирается в тишину: ключ его вкладки лежит
// в браузере, и молчание переживает перезагрузку страницы. Именно так
// на боевой машине разговор, попавший в очередь, оставался немым днями.
//
// Проверяет границу assistantSilent, и она одна на все три двери, где
// вопрос задаётся.
//
// ————— чего здесь нет —————
//
...
public class ChatDesk {
    ...
    public Thread say(String visitorKey, String text, String intent, Context context) {
        ...
        if (assistantSilent(conversation)) return thread(conversation);
    ...
    public void answered(UUID conversationId, AskReply reply) {
        ...
        if (assistantSilent(conversation)) return;
    ...
    /**
     * Молчит ли Ведалина в этом разговоре.
     *
     * <p>Молчит, когда в разговоре есть человек. Определяется это двумя
     * признаками, и оба означают «есть кому перебить»:
     *
     * <ul>
     *   <li>разговор кем-то взят ({@code owner != null}) — дальше говорит он;</li>
     *   <li>открыто хоть одно рабочее место — дежурный смотрит в очередь
     *       прямо сейчас и вот-вот возьмёт разговор; справка по каталогу,
     *       вылезшая между его «сейчас уточню» и ответом посетителя,
     *       выглядит как сотрудник, не читающий, что ему пишут.</li>
     * </ul>
     *
     * <p><b>А очередь, которую некому взять, молчания не требует.</b>
     * Разговор в WAITING без владельца и без единого открытого рабочего
     * места — это не «передан человеку», а «положен в очередь, которую
     * сейчас никто не читает». Перебивать там некого, а посетитель упирается
     * в вечную тишину: ключ вкладки лежит у него в браузере, и немота
     * переживает перезагрузку страницы. На боевой машине такой разговор
     * молчал днями.
     *
     * <p>Статус при этом не трогается: разговор остаётся в WAITING, из
     * очереди не уходит, и пометка «ждём специалиста» у посетителя стоит
     * прежняя. Ведалина здесь не отменяет передачу, а просто не молчит,
     * пока та не состоялась.
     */
    private boolean assistantSilent(Conversation conversation) {
        if (!conversation.handedToHuman()) return false;
        return conversation.getOwner() != null || stream.staffOnline();
    }
