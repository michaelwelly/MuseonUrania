...
public record LeadSubmission(
        ...
        @Schema(description = """
                Изделие, из карточки которого отправлена заявка.

                Обязательно в сервисной форме (`form: service`): обращение «что-то
                сломалось» без изделия инженеру нечего взять в работу. В остальных
                четырёх формах необязательно — там изделия у человека ещё нет.""",
                example = "vedal-r1-r2")
        String productSlug,

        @Schema(description = """
                Серийный номер изделия. Показывается только в сервисной форме и там же
                обязателен: номер — единственное, что отличает конкретный аппарат
                от модели, а сервис едет к аппарату. В остальных формах поля нет вовсе:
                в запросе цены или каталога изделия у человека ещё нет.
