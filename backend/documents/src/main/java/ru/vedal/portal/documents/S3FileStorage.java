...
// Полная реализация порта. Хранилище — Yandex Object Storage, одно и то же
// и локально, и в облаке: различаются ключи и, при желании, имена бакетов.
//
...
public class S3FileStorage implements FileStorage {
    ...
    public void put(Area area, String key, InputStream data, long size, String contentType) {
        ...
        // не всплывало: MinIO, который тогда стоял рядом, был открыт на
        // чтение политикой бакета, и приватный объект всё равно отдавался.
        // На ВМ политики нет — доступ выдан пообъектно, — и снимок,
        // загруженный редактором через админку, лёг бы приватным. Сайт
        // показал бы вместо него пустое место, а в журнале приложения при
        // этом стояло бы «файл сохранён».
        //
