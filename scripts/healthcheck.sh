...
while read -r name status; do
    # Порядок веток тут решает всё, и первая написана не для красоты.
    #
    # Раньше их было две: `Up*healthy*|Up*` — и обе принимали что угодно,
    # начинающееся с «Up». Слово «unhealthy» содержит «healthy», поэтому
    # маска `Up*healthy*` совпадала с «Up 12 days (unhealthy)», и сторож
    # считал больной контейнер здоровым.
    #
    # Так и вышло: Debezium был нездоров двенадцать суток, а сторож молчал.
    # Стоило это дорого — упавший коннектор уронил выкатку в бесконечный
    # цикл, и стенд пересобирался каждые две минуты, отдавая 502.
    case "$status" in
        *'(unhealthy)'*) problems+=("контейнер $name нездоров: $status") ;;
        Up*) ;;
